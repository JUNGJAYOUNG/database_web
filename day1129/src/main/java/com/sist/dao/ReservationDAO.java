...
public class ReservationDAO {
	...
	public String insertReservation(ReservationVO r) {
		...
		try {
			...
			pstmt.setString(1, getReservationNo());
	...
	public void updateTotal(String reservation_no) {
		System.out.println("동작함"+reservation_no);
		String sql = "update reservation set total= (select fee*(select checkout-checkin from reservation where reservation_no=?) from room where room_no = (select room_no from reservation where reservation_no=?) "
